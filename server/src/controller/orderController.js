...
// UPDATE order - النسخة المؤمنة ضد أخطاء الـ Null والـ OTP
exports.updateOrder = async (req, res) => {
  try {
    const { id } = req.params;
    const body = req.body;
    ...
    // --- الجزء المعدل لإرسال الـ OTP بأمان ---
    if (updates.status === "Confirmed") {
      try {
        // فحص وجود اليوزر ورقم الهاتف قبل محاولة القراءة (هون كان الخطأ)
        if (updatedOrder.userId && updatedOrder.userId.phone) {
          const { sendOrderConfirm } = require("../utils/otp");
          await sendOrderConfirm(updatedOrder.userId.phone);
          console.log(`OTP sent successfully to: ${updatedOrder.userId.phone}`);
        } else {
          console.warn(`Order ${id} updated but user or phone is missing. Skipping OTP.`);
        }
      } catch (otpErr) {
        // لو فشل الـ OTP لا نريد تعطيل استجابة الدفع
        console.error("OTP Error (Non-critical):", otpErr.message);
      }
    }
    ...
    // الرد دائما بـ 200 إذا وصل الكود لهون
    res.status(200).json(updatedOrder);
  } catch (err) {
    console.error("updateOrder critical error:", err);
    res.status(500).json({ message: err.message });
  }
};