...
exports.getTodayOrders = async (req, res) => {
  try {
    ...
    const orders = await Order.find({ createdAt: { $gte: start, $lte: end } })
      ...
      .populate(
        "products.productId",
        "name image additions basePrice prices discount",
      )
      .populate("userId", "phone")
      .populate("shippingAddress", "name deliveryCost")
      .lean()
...
// CREATE order
// INTERNAL helper for creating an order without HTTP response mapping
exports.createOrderLogic = async ({
  userId,
  products,
  status,
  shippingAddress,
...
exports.createOrder = async (req, res) => {
  try {
    ...
    const populatedOrder = await exports.createOrderLogic({
      ...req.body,
      io,
    });
