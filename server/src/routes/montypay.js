...
async function createPendingOrder({
  ...
  const newOrder = await Order.create({
    ...
    orderType,
    userDetails,
    // ✅ نحفظ كود الخصم هون وقت الإنشاء (لسا الطلب unpaid)
    // عشان نقدر نرجع نستخدمه وقت ما الدفع يتأكد فعليًا بالـ callback/verify
...
router.post("/callback", async (req, res) => {
  try {
    ...
    if (isPaid) {
      ...
      if (dbOrderId) {
        // ✅ حماية من التنفيذ المزدوج: إذا MontyPay بعتت الـ webhook أكثر من
        // مرة (بيصير كتير بمعظم بوابات الدفع)، ما منعيد تحديث الطلب ولا
        // منسجل استخدام كود الخصم مرتين.
        const existingOrder = await Order.findById(dbOrderId);
        if (!existingOrder) {
          console.warn(`⚠️  Callback: order ${dbOrderId} not found in DB.`);
        ...
        if (existingOrder.payment?.status === "paid") {
          console.log(`ℹ️  Callback: order ${dbOrderId} already marked paid, skipping.`);
          return res.status(200).send("OK");
        }
        ...
        if (updatedOrder) {
          ...
          // ✅ تسجيل استخدام كود الخصم (كان محفوظ بالأوردر وقت الإنشاء)
          if (updatedOrder.promoCode) {
            try {
              ...
              console.error(
                "=== PROMO USAGE RECORD ERROR (montypay callback) ===",
                promoErr.message,
              );
            }
          }
...
router.post("/verify", async (req, res) => {
  try {
    ...
    if (existingOrder.payment?.status === "paid") {
      // Already confirmed — nothing to do (promo usage already recorded by whoever confirmed first)
      return res.json({ success: true, alreadyConfirmed: true });
    }
    ...
    if (updatedOrder) {
      ...
      if (updatedOrder.promoCode) {
        try {
          ...
          console.error(
            "=== PROMO USAGE RECORD ERROR (montypay verify) ===",
            promoErr.message,
          );
        }
      }
