...
//       statusCheck?.StatusCode === 2 && statusCheck?.StatusMessageEn === "Paid";
...
router.post("/confirm", async (req, res) => {
  try {
    ...
    // ✅ تسجيل استخدام كود الخصم (إذا كان موجود) بعد ما تأكد الدفع والأوردر انحفظ
    if (orderData.promoCode) {
      try {
        await recordPromoUsage(orderData.promoCode, orderData.userId, order._id);
      } catch (promoErr) {
        console.error("=== PROMO USAGE RECORD ERROR (orange) ===", promoErr.message);
        // ما منوقف الطلب بسبب فشل تسجيل الاستخدام، بس منسجله باللوق للمتابعة
      }
    }
