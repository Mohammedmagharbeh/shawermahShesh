...
// // ─── Shared helpers (same logic as montypay / orderController) ───────────────
...
// /**
//  * Creates an order document in MongoDB (same as MontyPay's createPendingOrder).
//  */
...
//   // Emit newOrder — payment is already confirmed at this point
//   if (io) {
...
// // ─── 1) Initiate: Send OTP to user's mobile ─────────────────────────────────
...
// // ─── 2) Confirm: Verify OTP + Create Order ───────────────────────────────────
...
//     // // Step 1: Confirm payment with ZainCash SOAP API
//     const result = await confirmPayment({ amount, mobile, otp, note });

//     // Check for ZainCash-level errors
...
//     // Check ErrorObj (ErrorCode "0" = success)
//     if (result?.ErrorObj && result.ErrorObj.ErrorCode !== "0") {
//       return res.status(400).json({
//         message: "فشل تأكيد عملية الدفع",
//         detail: result.ErrorObj.ErrorMessage || "Payment verification failed",
//       });
//     }

//     // Step 2: Payment confirmed — create the order in DB
//     let order = null;
//     if (orderData) {
//       try {
...
//         console.log(`✅ ZainCash order ${order._id} created and paid.`);
//       } catch (orderError) {
//         console.error("ZainCash order creation error:", orderError.message);
//         // Payment succeeded but order creation failed — return partial success
//         // so the user knows payment went through
...
router.post("/zain/confirm", async (req, res) => {
  try {
    ...
    console.log("[ZainCash] Confirm raw result:", JSON.stringify(result, null, 2));
    ...
    if (orderData) {
      try {
        ...
        console.log(`✅ ZainCash order ${order._id} created and paid. RefID: ${transactionId}`);
      } catch (orderError) {
        console.error("[ZainCash] Order creation error:", orderError.message);
