...
export const useCheckoutLogic = (t) => {
  ...
  // ✅ cliqPhone هون كـ field مستقل، مش داخل details
  const [formState, setFormState] = useState(() => ({
    ...DEFAULT_FORM_STATE,
    cliqPhone: "",
    details: {
  ...
  // ✅ onSendOtp بعد updateForm مباشرة
  /**
   * Saves the CliQ phone number entered by the user in the modal
   */
  const onSendOtp = useCallback(
    (phoneNumber) => {
      updateForm("cliqPhone", phoneNumber);
      // هون تقدر تضيف API call لإرسال OTP إذا بدك
      // مثلاً: await PaymentService.sendCliqOtp(phoneNumber);
    },
    [updateForm],
  );
  ...
  /**
   * Handles ZainCash (CliQ) payment initiation
   * ✅ بتستخدم cliqPhone بدل details.phone
   */
  const handleZainCashFlow = useCallback(async () => {
    try {
      await PaymentService.zainCash.initiate({
        orderSummary,
        phone: formState.cliqPhone, // ✅ رقم محفظة CliQ
      });
  ...
  }, [orderSummary, formState.cliqPhone]); // ✅ dependency محدّثة

  /**
   * Confirms ZainCash payment with OTP
   * ✅ بتبعت cliqPhone للـ API
   */
  const confirmCliqPayment = useCallback(async () => {
    setIsSubmitting(true);
