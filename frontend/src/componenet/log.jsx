...
// //     // --- كود تجريبي لاختبار رسالة الاعتذار الآن ---
// //     const now = new Date();
// //     const currentHour = now.getHours(); // الساعة الآن (مثلاً 15)
// //     const currentMinute = now.getMinutes(); // الدقائق الآن

// //     // هذا الشرط سيفعل رسالة الإغلاق إذا كانت الساعة 3 عصراً (15) 
// //     // يمكنك تغيير الرقم 15 حسب الساعة الحالية عندك بالضبط لتجربتها
// //     if (currentHour === 15) { 
// //       toast.error(
// //         "عذراً زبوننا العزيز.. مطعمنا مغلق الآن. نحن نأخذ استراحة قصيرة لنعود لخدمتكم بكل حب من الساعة 9 صباحاً. ننتظركم بشوق!",
// //         {
// //           duration: 5000,
// //           icon: '🌙',
// //           style: {
...
// //   //   // 1. منطق ساعات العمل (من 3 فجراً حتى 9 صباحاً)
// //   //   const now = new Date();
// //   //   const currentHour = now.getHours(); // يجلب الساعة بنظام 24 ساعة
...
// //   //   // 2. منطق الحظر المؤقت (الكود السابق الذي جربناه)
...
// //   //     toast.error(`يرجى الانتظار ${remainingSeconds} ثانية للمحاولة مجدداً.`);
// //   //     return;
// //   //   }
...
//     // 1. منطق ساعات عمل المطعم (من 3 فجراً حتى 9 صباحاً)
//     const now = new Date();
...
//     // 2. التحقق من الحظر المؤقت (localStorage) إذا وجد
//     const lockUntil = localStorage.getItem("login_lock_until");
//     const currentTime = new Date().getTime();
...
//       if (res.data.token) {
//         // حذف أي حظر سابق عند نجاح الدخول
...
//               whileTap={!loading && phone.length === 10 ? { scale: 0.98 } : {}}
//               className={`w-full py-4 rounded-xl font-bold text-lg transition-all duration-200
//                 bg-yellow-400 text-black shadow-yellow-500/30 hover:bg-yellow-500 hover:shadow-xl hover:shadow-yellow-600/50 active:bg-yellow-600
//               `}
//               disabled={loading}
//             >
//               {loading ? t("logging_in") + "..." : t("login")}
...
//   // التحقق من حالة فتح/إغلاق المطعم وتنسيق الخلفية
//   useEffect(() => {
//     const checkStatus = () => {
//       const hour = new Date().getHours();
//       // مغلق من الساعة 3:00 فجراً حتى 8:59 صباحاً
//       if (hour >= 3 && hour < 9) {
//         setIsOpen(false);
//       } else {
//         setIsOpen(true);
...
//     checkStatus();
//     // تحديث الحالة كل دقيقة للتأكد من دقة الوقت
...
//     // 1. منع تسجيل الدخول إذا كان المطعم مغلقاً
//     if (!isOpen) {
//       toast.error(
//         "عذراً زبوننا العزيز.. مطعمنا مغلق الآن. نحن نأخذ استراحة قصيرة لنعود لخدمتكم بكل حب من الساعة 9 صباحاً. ننتظركم بشوق!",
//         {
...
//     // 2. التحقق من الحظر المؤقت (localStorage)
//     const lockUntil = localStorage.getItem("login_lock_until");
//     const currentTime = new Date().getTime();
...
//               </span>
//               <span className="text-green-100 text-xs font-bold uppercase tracking-wider">نستقبل طلباتكم الآن</span>
//             </div>
//           ) : (
//             <div className="flex items-center gap-2 bg-black/30 px-4 py-1.5 rounded-full border border-white/20">
//               <span className="h-3 w-3 rounded-full bg-gray-400"></span>
//               <span className="text-gray-200 text-xs font-bold">المطعم مغلق - يفتح 9:00 صباحاً</span>
//             </div>
//           )}
//         </div>
...
//               disabled={loading}
//             >
//               {loading ? t("logging_in") + "..." : isOpen ? t("login") : "المطعم مغلق حالياً"}
//             </motion.button>
//           </div>
//         </form>
//       </motion.div>
