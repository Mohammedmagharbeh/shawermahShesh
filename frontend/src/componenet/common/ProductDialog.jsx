...
export function ProductDialog({ id, triggerLabel }) {
  ...
  const getCurrentPrice = () => {
    if (product.hasProteinChoices && product.hasTypeChoices) {
      return product.prices?.[selectedProtein]?.[selectedType] || product.basePrice || 0;
    }
    if (product.hasTypeChoices) return product.prices?.[selectedType] || product.basePrice || 0;
    if (product.hasProteinChoices) return product.prices?.[selectedProtein] || product.basePrice || 0;
    return product.basePrice || 0;
  };
  ...
  const handleAddToCart = () => {
    ...
    const selectedFullAdditions = product.additions
      ?.filter((a) => selectedAdditions.includes(a._id))
      .map((a) => ({
        name: a.name, // { ar, en }
