...
//   const fetchJobs = async () => {
//     try {
//       // تعديل المسار ليطابق الباك إند الجديد
//       const res = await axios.get(`${import.meta.env.VITE_BASE_URL}/jobs`);
//       setJobs(res.data);
//     } catch (err) {
//       toast.error(t("failed_load_jobs"));
...
//     const formData = new FormData();
//     for (const key in form) {
//       // نرسل الملفات فقط إذا كانت موجودة
//       if (form[key] !== null) {
//         formData.append(key, form[key]);
//       }
//     }
...
//     try {
//       // تعديل الرابط ليتناسب مع الباك إند الجديد /jobs/apply
//       await axios.post(`${import.meta.env.VITE_BASE_URL}/jobs/apply`, formData, {
//         headers: { "Content-Type": "multipart/form-data" },
//       });
      
//       toast.success(t("application_success"));
      
//       // تصغير الفورم وإعادة التصفير بعد النجاح
//       setForm({
//         applicantName: "",
//         applicantEmail: "",
//         phone: "",
...
//     } catch (err) {
//       console.log(err);
//       // فحص إذا كان المستخدم محظور (Blacklisted)
//       if (err.response && err.response.status === 403) {
//         toast.error(t("blacklisted_error_msg") || "This email is blacklisted from applying.");
//       } else {
//         toast.error(t("application_error"));
//       }
//     }
...
//                 <label className="font-bold text-[#DA0103]" htmlFor="start-date">{t("start_date")}</label>
//                 <input
//                   type="date"
//                   name="start-date"
//                   id="start-date"
...
//                     setForm({ ...form, startDate: e.target.value })
//                   }
//                   className="px-4 py-3 border-2 border-[#FFC400] rounded-lg bg-white focus:outline-none focus:border-[#DA0103] focus:ring-2 focus:ring-[#DA0103]/20 transition-all"
//                 />
//               </div>
//             </div>
...
//               {jobs.map((job) => (
//                 <div key={job._id} className="bg-white border-2 border-[#DA0103] border-r-4 border-r-[#FFC400] rounded-lg p-5 shadow-md">
//                   <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
//                     <div className="flex-1">
...
//                         {/* عرض الجنس المطلوب الذي حدده الأدمن */}
//                         <p className="bg-red-50 px-2 py-0.5 rounded border border-red-100">
//                           <span className="font-bold text-[#DA0103]">{t("required_gender")}:</span>{" "}
...
//                     <button
//                       onClick={() => setForm({ ...form, jobId: job._id })}
//                       className="w-full sm:w-auto bg-[#FFC400] text-[#DA0103] font-bold py-3 px-8 rounded-lg hover:bg-[#DA0103] hover:text-[#FFC400] transition-all active:scale-95"
//                     >
//                       {t("apply_job")}
//                     </button>
//                   </div>
...
//         {/* Application Form Section */}
//         {form.jobId && (
//           <div className="bg-white border-2 border-[#DA0103] rounded-lg p-6 sm:p-8 shadow-lg animate-in fade-in duration-500">
//             <h3 className="text-2xl font-bold text-[#DA0103] pb-4 border-b-2 border-[#FFC400] mb-6">
//               {t("apply_for_job")}
//             </h3>
...
//                 className="px-4 py-3 border-2 border-[#FFC400] rounded-lg focus:outline-none focus:border-[#DA0103]"
//               />
//               <input
//                 placeholder={t("your_email")}
//                 type="email"
...
//               <div className="flex flex-col gap-2 col-span-1 sm:col-span-2">
//                 <label className="font-bold text-[#DA0103]">{t("start_date")}</label>
//                 <input
//                   type="date"
//                   value={form.startDate}
...
//               <div className="p-4 bg-red-50 border-l-4 border-[#DA0103] rounded-lg">
//                 <label className="block text-[#DA0103] font-bold mb-2">{t("resume")}</label>
//                 <input
//                   type="file"
//                   accept=".pdf,.doc,.docx"
//                   onChange={(e) => handleFileChange(e, "resume")}
//                   className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-[#FFC400] file:text-[#DA0103] file:font-bold hover:file:bg-[#DA0103] hover:file:text-[#FFC400]"
//                 />
//               </div>
//               <div className="p-4 bg-red-50 border-l-4 border-[#DA0103] rounded-lg">
//                 <label className="block text-[#DA0103] font-bold mb-2">{t("experience_certificate")}</label>
//                 <input
//                   type="file"
//                   accept=".pdf,.doc,.docx"
//                   onChange={(e) => handleFileChange(e, "experienceCertificate")}
...
//               <label className="block text-[#DA0103] font-bold mb-4">{t("worked_before")}</label>
//               <div className="flex gap-8 mb-4">
//                 <label className="flex items-center gap-2 cursor-pointer">
//                   <input type="radio" checked={form.workedBefore === "yes"} onChange={() => setForm({ ...form, workedBefore: "yes" })} className="accent-[#DA0103]" />
//                   <span>{t("yes")}</span>
//                 </label>
//                 <label className="flex items-center gap-2 cursor-pointer">
//                   <input type="radio" checked={form.workedBefore === "no"} onChange={() => setForm({ ...form, workedBefore: "no" })} className="accent-[#DA0103]" />
//                   <span>{t("no")}</span>
//                 </label>
//               </div>
...
//                   <input placeholder={t("previous_workplace")} value={form.previousJobs} onChange={(e) => setForm({ ...form, previousJobs: e.target.value })} className="px-4 py-3 border-2 border-[#FFC400] rounded-lg bg-white" />
//                   <input placeholder={t("previous_title")} value={form.previousTitle} onChange={(e) => setForm({ ...form, previousTitle: e.target.value })} className="px-4 py-3 border-2 border-[#FFC400] rounded-lg bg-white" />
//                 </div>
//               )}
//             </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:py-12">
        ...
        <div className="mb-12">
          ...
          {jobs.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6">
              {jobs.map((job) => (
                <div key={job._id} className="bg-white border-2 border-[#DA0103] border-r-4 border-r-[#FFC400] rounded-lg p-5 shadow-md hover:shadow-xl transition-shadow">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-[#DA0103] mb-2">
                        {currentLang === "ar" ? job.title.ar : job.title.en}
            ...
            <div className="mt-6 p-4 bg-red-50 border-2 border-dashed border-[#FFC400] rounded-lg">
              ...
              <div className="flex gap-8 mb-4">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="radio" checked={form.workedBefore === "yes"} onChange={() => setForm({ ...form, workedBefore: "yes" })} className="accent-[#DA0103] w-4 h-4" />
                  <span className="font-semibold group-hover:text-[#DA0103] transition-colors">{t("yes")}</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="radio" checked={form.workedBefore === "no"} onChange={() => setForm({ ...form, workedBefore: "no" })} className="accent-[#DA0103] w-4 h-4" />
                  <span className="font-semibold group-hover:text-[#DA0103] transition-colors">{t("no")}</span>
                </label>
              </div>

              {form.workedBefore === "yes" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in slide-in-from-top-2 duration-300">
                  <input placeholder={t("previous_workplace")} value={form.previousJobs} onChange={(e) => setForm({ ...form, previousJobs: e.target.value })} className="px-4 py-3 border-2 border-[#FFC400] rounded-lg bg-white focus:outline-none focus:border-[#DA0103]" />
                  <input placeholder={t("previous_title")} value={form.previousTitle} onChange={(e) => setForm({ ...form, previousTitle: e.target.value })} className="px-4 py-3 border-2 border-[#FFC400] rounded-lg bg-white focus:outline-none focus:border-[#DA0103]" />
                </div>
              )}
            </div>
