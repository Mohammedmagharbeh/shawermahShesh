...
//     try {
//       const res = await axios.get(
//         `${import.meta.env.VITE_BASE_URL}/jobs?admin=true`,
//       );
//       setJobs(res.data);
//     } catch (err) {
//       toast.error(t("failed_to_load_jobs"));
...
//         prev.map((app) =>
//           app._id === appId ? { ...app, status: "seen" } : app,
//         ),
//       );
//       await axios.patch(
//         `${import.meta.env.VITE_BASE_URL}/jobs/application-status/${appId}`,
//         { status: "seen" },
//       );
//     } catch (err) {
...
//         `${import.meta.env.VITE_BASE_URL}/jobs/blacklist-toggle`,
//         { email },
//       );
...
//           <button
//             onClick={() => toast.dismiss(tObj.id)}
//             className="px-3 py-1 bg-gray-200 rounded text-sm font-bold"
//           >
//             {t("no")}
//           </button>
//         </div>
...
//     try {
//       await axios.patch(
//         `${import.meta.env.VITE_BASE_URL}/jobs/toggle-status/${jobId}`,
//       );
...
//           <button
//             onClick={addJob}
//             className="bg-[#DA0103] text-white px-6 md:px-8 py-3 md:py-4 rounded-lg font-bold mt-2 hover:bg-red-700 transition-all shadow-lg active:scale-95"
//           >
//             {t("add_job")}
//           </button>
//         </div>
...
//             <div
//               key={job._id}
//               className={`bg-white border-2 border-[#FFC400] border-l-8 rounded-lg shadow-md transition-shadow ${job.isActive === false ? "opacity-60 border-l-gray-400" : "border-l-[#DA0103]"}`}
//             >
...
//                 <p className="text-gray-700 mb-3">{job.type[userLang]}</p>
//                 <div
//                   className={`flex flex-wrap gap-3 ${i18n.language === "ar" ? "flex-row-reverse" : ""}`}
//                 >
//                   <button
//                     onClick={() => viewApplications(job._id)}
//                     className="bg-[#FFC400] text-gray-900 px-4 py-2 rounded-lg font-bold text-sm md:text-base hover:bg-yellow-300"
//                   >
//                     {t("view_applicants")}
//                   </button>
//                   <button
//                     onClick={() => toggleJobVisibility(job._id)}
//                     className="bg-gray-800 text-white px-4 py-2 rounded-lg font-bold text-sm md:text-base hover:bg-black"
//                   >
//                     {job.isActive ? "إخفاء" : "إظهار"}
//                   </button>
//                   <button
//                     onClick={() => confirmDeleteJob(job._id)}
//                     className="bg-[#DA0103] text-white px-4 py-2 rounded-lg font-bold text-sm md:text-base hover:bg-red-700"
//                   >
//                     {t("delete_job")}
//                   </button>
//                 </div>
...
//             <h3 className="text-2xl md:text-3xl font-bold text-[#DA0103]">
//               {t("job_applicants")}
//             </h3>
//             <button
...
//                   app.isBlacklisted
//                     ? "bg-red-50 border-red-300 border-l-black"
//                     : app.status === "seen"
//                       ? "bg-gray-50 border-gray-200 border-l-gray-400 opacity-90"
...
//                     <b className="text-[#DA0103]">{t("start_date")}:</b>{" "}
//                     {app.startDate
//                       ? new Date(app.startDate).toLocaleDateString("ar-JO")
//                       : "---"}
//                   </p>
//                   <p className="text-sm md:text-base">
//                     <b className="text-[#DA0103]">{t("worked_before")}:</b>{" "}
...
//                 <div
//                   className={`flex flex-col sm:flex-row gap-4 mt-6 pt-4 border-t border-gray-100 ${i18n.language === "ar" ? "flex-row-reverse" : ""}`}
//                 >
//                   <div className="flex-1">
...
//                         target="_blank"
//                         rel="noopener noreferrer"
//                         onClick={() => markAsSeen(app._id, app.status)}
//                         className="inline-block bg-[#DA0103] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-700"
//                       >
//                         📄 {t("view_file")}
//                       </a>
//                     ) : (
...
//                         rel="noopener noreferrer"
//                         onClick={() => markAsSeen(app._id, app.status)}
//                         className="inline-block bg-[#FFC400] text-gray-900 px-4 py-2 rounded-lg text-sm font-bold hover:bg-yellow-300"
//                       >
//                         📜 {t("view_file")}
//                       </a>
//                     ) : (
...
export default function AdminJobs() {
  ...
  const [form, setForm] = useState({
    ...
    typeAr: "",
    typeEn: "",
    genderAr: "كلاهما", // القيمة الافتراضية بالعربي
    genderEn: "Both",   // القيمة الافتراضية بالإنجليزي
  });
  const [selectedJob, setSelectedJob] = useState(null);
  const [applications, setApplications] = useState([]);
  ...
  const addJob = async () => {
    ...
    try {
      // هنا الـ form صار يحتوي على genderAr و genderEn تلقائياً
      await axios.post(`${import.meta.env.VITE_BASE_URL}/jobs`, form);
      toast.success(t("job_added_success"));
      // تصغير الـ form ورجوعه للحالة الأصلية
      setForm({ 
        titleAr: "", 
        titleEn: "", 
        typeAr: "", 
    ...
    >
      ...
      <div className="bg-white border-2 border-[#DA0103] rounded-lg p-6 md:p-8 mb-8 shadow-lg">
        ...
        <div className="flex flex-col gap-4 mt-6">
          ...
          {/* حقل اختيار الجنس الجديد */}
          <div>
            <label className="text-gray-900 font-bold block mb-2 text-sm md:text-base">
              {userLang === "ar" ? "الجنس المطلوب" : "Required Gender"}
            </label>
            <select
              value={form.genderAr}
              onChange={(e) => {
                ...
                let enVal = "Both";
                if (val === "ذكر") enVal = "Male";
                if (val === "أنثى") enVal = "Female";
                setForm({ ...form, genderAr: val, genderEn: enVal });
              }}
              className="w-full px-4 py-3 md:py-4 border-2 border-[#FFC400] rounded-lg text-base md:text-lg focus:outline-none focus:border-[#DA0103] transition-all bg-white cursor-pointer"
            >
              <option value="كلاهما">{userLang === "ar" ? "كلاهما" : "Both"}</option>
              <option value="ذكر">{userLang === "ar" ? "ذكر" : "Male"}</option>
              <option value="أنثى">{userLang === "ar" ? "أنثى" : "Female"}</option>
            </select>
          </div>
          ...
          {jobs.map((job) => (
            ...
            >
              <div className="p-4 md:p-6">
                ...
                <p className="text-gray-700 mb-1">{job.type[userLang]}</p>
                {/* عرض الجنس في القائمة */}
                <p className="text-sm font-bold text-gray-500 mb-3 italic">
                  {userLang === "ar" ? "الجنس:" : "Gender:"} {job.gender?.[userLang] || (userLang === "ar" ? "كلاهما" : "Both")}
                </p>
