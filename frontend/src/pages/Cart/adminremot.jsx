...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Plus, Search, Edit2, Trash2, Package, DollarSign, ImageIcon, Tag, Loader2 } from "lucide-react"
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="مثال: برجر كلاسيك" required className="h-11" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="price" type="number" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} placeholder="45" required className="h-11" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="category" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} placeholder="مثال: برجر، بيتزا، مشروبات" required className="h-11" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="image" value={formData.image} onChange={(e) => setFormData({ ...formData, image: e.target.value })} placeholder="https://example.com/image.jpg" required className="h-11" />
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="description" className="text-sm font-medium">الوصف</Label>
                    <Textarea id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="وصف تفصيلي للمنتج..." required rows={4} className="resize-none" />
                  </div>
                  ...
                  {editingId && (
                    <Button type="button" variant="outline" onClick={() => { setEditingId(null); setFormData({ name: "", price: "", description: "", image: "", category: "" }) }} className="w-full h-11">
                      إلغاء التعديل
                    </Button>
                  )}
                </form>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-border/50 shadow-lg">
              <CardContent className="pt-6">
                <div className="relative mb-4">
                  <Search className="absolute right-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                  <Input placeholder="ابحث عن منتج..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pr-10 h-11" />
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {categories.map((cat, i) => (
                    <Button
                      ...
                      onClick={() => { setSelectedCategory(cat); setProductsToShow(PRODUCTS_PER_PAGE) }}
                      className={`rounded-full px-4 py-2 ${selectedCategory === cat ? "bg-primary text-white" : "border border-primary text-primary"}`}
                    >
                      {cat}
                    </Button>
                  ))}
            ...
            {hasMoreProducts && (
              <div className="flex justify-center mt-6">
                <Button onClick={() => setProductsToShow(productsToShow + PRODUCTS_PER_PAGE)}>
                  إظهار المزيد
                </Button>
              </div>
            )}
