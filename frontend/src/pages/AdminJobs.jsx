...
export default function AdminJobs() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8 font-sans" dir={i18n.language === "ar" ? "rtl" : "ltr"}>
      ...
      {/* Header */}
      <div className="text-center mb-8 pb-6 border-b-4 p-4 sm:pt-10 md:pt-16 lg:pt-20 border-[#DA0103]">
          ...
          <div className="grid grid-cols-1 gap-4 md:gap-6">
            {jobs.map((job) => (
              ...
              >
                ...
                <div className={`flex flex-col sm:flex-row gap-3 md:gap-4 ${i18n.language === "ar" ? "flex-row-reverse" : ""}`}>
                  <button
      ...
      {selectedJob && (
        <div className="mb-12">
          ...
          {applications.length === 0 ? (
            <div className="bg-white border-2 border-dashed border-[#FFC400] p-8 md:p-12 text-center rounded-lg">
              <p className="text-gray-600 text-lg md:text-xl">{t("no_applicants_yet")}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 md:gap-6">
              {applications.map((app) => (
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-4">
                    ...
                    <p className="text-sm md:text-base"><b className="text-[#DA0103]">{t("phone")}:</b> {app.phone}</p>
                    ...
                    <p className="text-sm md:text-base">
                      <b className="text-[#DA0103]">{t("start_date")}:</b> {new Date(app.startDate).toLocaleDateString(i18n.language === "ar" ? "ar-SA" : "en-US")}
                    </p>
                  </div>

                  <p className="text-sm md:text-base mb-3">
                    <b className="text-[#DA0103]">{t("worked_before")}:</b>{" "}
                    <span className={`ms-2 px-3 py-1 rounded-full text-sm font-bold text-white inline-block ${app.workedBefore === "yes" ? "bg-[#DA0103]" : "bg-gray-600"}`}>
                  ...
                  <div className={`flex flex-col sm:flex-row gap-3 md:gap-4 ${i18n.language === "ar" ? "flex-row-reverse" : ""}`}>
                    <div className="flex-1">
                      ...
                      {app.resume ? (
                        <a href={app.resume} target="_blank" rel="noopener noreferrer" className="inline-block bg-[#DA0103] text-white px-3 md:px-4 py-2 rounded-lg text-sm md:text-base font-bold hover:bg-red-700 transition-colors">
                          📄 {t("view_file")}
                        </a>
                      ) : (
                        <span className="text-gray-500 text-sm">لم يرفق</span>
                      )}
                    </div>
                    <div className="flex-1">
                      <b className="text-[#DA0103] block mb-2">{t("experience_certificate")}:</b>
                      {app.experienceCertificate ? (
                        <a href={app.experienceCertificate} target="_blank" rel="noopener noreferrer" className="inline-block bg-[#FFC400] text-gray-900 px-3 md:px-4 py-2 rounded-lg text-sm md:text-base font-bold hover:bg-yellow-300 transition-colors">
                          📜 {t("view_file")}
                        </a>
                      ) : (
                        <span className="text-gray-500 text-sm">لم يرفق</span>
