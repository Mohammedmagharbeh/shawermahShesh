...
// import { Users, ShoppingCart, TrendingUp, Phone, CheckCircle2, XCircle, Search, Loader2, BarChart3 } from "lucide-react"
...
//       // جلب البيانات الأساسية فقط - النقاط الموجودة فعلياً
//       const usersResponse = await axios.get("http://localhost:5000/api/users")
//       const ordersResponse = await axios.get("http://localhost:5000/api/order/get")
...
//         console.log("عينة من هيكل الطلب الأول:", ordersData[0])
//         console.log("المفاتيح الموجودة في الطلب:", Object.keys(ordersData[0]))
//       }

//       // معالجة البيانات
//    const processedUsers = usersData.map(user => {
//   // جلب جميع الطلبات الخاصة بالمستخدم بناءً على userId
//   const userOrders = ordersData.filter(order => order.userId === user._id);

//   // حساب إجمالي الإنفاق
//   const totalSpent = userOrders.reduce((sum, order) => sum + (order.totalPrice || 0), 0);

//   // الحصول على الاسم
//   const name = user.name || userOrders[0]?.userDetails?.name || "غير معروف";
...
//       console.log("المستخدمين المعالجين:", processedUsers)
...
//       const conversionRate = processedUsers.length > 0 ? (usersWithOrders / processedUsers.length) * 100 : 0
//       const totalRevenue = processedUsers.reduce((sum, u) => sum + u.totalSpent, 0)
...
//               </div>
//               <div>
//                 <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold">إحصائيات الزوار</h1>
...
//                   <div className="flex items-center justify-between">
//                     <CardTitle className="text-base sm:text-lg">إجمالي الزوار</CardTitle>
...
//                   <div className="text-3xl sm:text-4xl font-bold text-blue-600">{stats.totalUsers}</div>
//                   <p className="text-xs sm:text-sm text-muted-foreground mt-1">عدد الأشخاص الذين وصلهم OTP</p>
//                 </CardContent>
//               </Card>
...
//                   <div className="flex items-center justify-between">
//                     <CardTitle className="text-base sm:text-lg">زوار قاموا بالطلب</CardTitle>
...
//                 </CardHeader>
//                 <CardContent className="pt-4">
//                   <div className="text-3xl sm:text-4xl font-bold text-green-600">{stats.usersWithOrders}</div>
...
//                   <div className="flex items-center justify-between">
//                     <CardTitle className="text-base sm:text-lg">زوار بدون طلبات</CardTitle>
...
//                 </CardHeader>
//                 <CardContent className="pt-4">
//                   <div className="text-3xl sm:text-4xl font-bold text-orange-600">{stats.usersWithoutOrders}</div>
//                   <p className="text-xs sm:text-sm text-muted-foreground mt-1">دخلوا ولم يطلبوا</p>
//                 </CardContent>
//               </Card>
...
//                   <div className="flex items-center justify-between">
//                     <CardTitle className="text-base sm:text-lg">إجمالي الطلبات</CardTitle>
...
//                 </CardHeader>
//                 <CardContent className="pt-4">
//                   <div className="text-3xl sm:text-4xl font-bold text-purple-600">{stats.totalOrders}</div>
//                   <p className="text-xs sm:text-sm text-muted-foreground mt-1">عدد الطلبات الكلي</p>
//                 </CardContent>
//               </Card>
...
//                   <div className="flex items-center justify-between">
//                     <CardTitle className="text-base sm:text-lg">إجمالي الإيرادات</CardTitle>
...
//                     {stats.totalRevenue.toFixed(2)} <span className="text-xl">د.أ</span>
//                   </div>
//                   <p className="text-xs sm:text-sm text-muted-foreground mt-1">مجموع قيمة جميع الطلبات</p>
//                 </CardContent>
//               </Card>
//             </div>
...
//             <Card>
//               <CardHeader>
//                 <CardTitle className="text-lg sm:text-xl">قائمة الزوار</CardTitle>
...
//                                 <p className="font-bold text-base sm:text-lg truncate">{user.phone}</p>
//                                 {user.name && <p className="text-sm text-muted-foreground truncate">{user.name}</p>}
...
//                                     <p className="font-bold text-sm">{user.orderCount}</p>
//                                   </div>
//                                   <div className="text-left">
