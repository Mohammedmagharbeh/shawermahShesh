...
//     toast.error(t("missing_payment_reference"));
//       setLoading(false);
//       return;
//     }
...
//   toast.error(t("order_creation_failed_try_again"));
//         }
//       } catch (e) {
...
function PaymentSuccess() {
  ...
  useEffect(() => {
    ...
    if (!paymentId) {
      setError(t("missing_payment_reference"));
      toast.error(t("missing_payment_reference"));
      setLoading(false);
      return;
    }
    ...
    verify();
  }, [location.search, t]); // ✅ أضفنا t هنا عشان ما يعطي تحذير React Hook
      ...
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl p-8 border border-emerald-100 text-center">
        {loading ? (
          <div>
            ...
            <p className="text-gray-600">{t("please_wait_payment")}</p>
          </div>
        ) : error ? (
          <div>
            <div className="mx-auto mb-6 w-16 h-16 flex items-center justify-center rounded-full bg-red-50 text-red-600 text-3xl">
          ...
          <div>
            ...
            <p className="text-gray-600">{t("thank_you_payment_confirmed")}</p>
            {invoiceId ? (
              <p className="mt-2 text-sm text-gray-500">
