...
import { UserPlus, Edit2, Save, X, Shield, Crown, User, Phone, Loader2 } from "lucide-react"

const AdminUsersPage = () => {
  const [users, setUsers] = useState([])
  const [showUsers, setShowUsers] = useState(false)
  const [newUser, setNewUser] = useState({ username: "", phone: "", role: "user" })
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-bold text-red-700 leading-tight">لوحة إدارة المستخدمين</h1>
              <p className="text-gray-600 mt-1">إدارة المستخدمين والصلاحيات بكل سهولة</p>
            </div>
          </div>
        ...
        <Card className="p-6 md:p-8 shadow-lg border-0 bg-white rounded-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                {actionLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <UserPlus className="w-5 h-5 ml-2" />}
                إضافة مستخدم
              </Button>
            </div>
          </div>
        ...
        {showUsers && users.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.map((user) => (
              <Card key={user._id} className="p-4 shadow-lg rounded-xl bg-white">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`px-2 py-1 rounded-xl ${getRoleBadgeColor(user.role)} text-sm flex items-center gap-1`}>
                ...
                <div className="mt-3">
                  {editUserId === user._id ? (
                    <div className="flex gap-2">
                      ...
                      <Button onClick={() => handleUpdateRole(user._id)} disabled={actionLoading} className="bg-red-700 text-white rounded-xl px-3">
                        حفظ
                      </Button>
                      <Button onClick={() => setEditUserId(null)} variant="outline" className="border border-gray-300 rounded-xl px-3">
                        إلغاء
                      </Button>
                    </div>
                  ) : (
                    <Button onClick={() => { setEditUserId(user._id); setEditRole(user.role) }} variant="outline" className="border border-red-700 text-red-700 rounded-xl px-3">
                      تعديل الصلاحية
                    </Button>
                  )}
                </div>
